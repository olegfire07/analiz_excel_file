# src/universal_excel_analyzer/app.py
...
app.title = "Универсальный Анализатор Excel (рус.)"
server = app.server
# ------------------------------------------------------------------------------
# КЭШ
# ------------------------------------------------------------------------------
...
def parse_contents(contents, filename):
    ...
    for sheet, data in df.items():
        ...
        df_serialized[sheet] = df_sheet.to_dict('records')
...
def combine_data(stored_data, sheet_names):
    ...
    for sheet in sheet_names:
        if sheet in stored_data:
            df_list.append(pd.DataFrame(stored_data[sheet]))
    if not df_list:
        return None
...
def apply_all_filters(df, filters):
    ...
    # Доп. фильтр (adv_col > adv_val)
    if adv_col and adv_col in df.columns and adv_val is not None:
        if pd.api.types.is_numeric_dtype(df[adv_col]):
...
def create_figure(df, x_col, y_col, chart_type, color_col, symbol_col):
    ...
    try:
        if chart_type == 'scatter' and y_col:
            ...
            # Для пузырьковых диаграмм используем размер пузырьков
            fig = px.scatter(
                df, x=x_col, y=y_col,
                color=color_col,
                size=y_col,  # Можно изменить на другой столбец для размера
...
def perform_pca(df, n_components):
    ...
    try:
        ...
        if n_components >= 2:
            ...
            fig_scatter = go.Figure().update_layout(title="Недостаточно компонентов для 2D-графика")
    ...
    if condition_col and condition_op and condition_val:
        try:
            if condition_op == 'contains':
                ...
                # Преобразование значения к числовому типу, если столбец числовой
...
def create_documentation():
    ...
    **Универсальный Анализатор Excel** — мощный инструмент для анализа данных из Excel-файлов. Ниже приведено краткое руководство по использованию приложения.
...
app.layout = dbc.Container([
    ...
    html.Div([
        html.H1("Универсальный Анализатор Excel", 
                className="text-center my-4", 
                style={"fontWeight": "bold", "fontSize": "2.5rem"}),
        html.H5("Кластеризация, Поиск, и Прочая Аналитика — всё по-русски", 
